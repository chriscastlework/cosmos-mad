namespace DatabaseMigrator.JsonCSharpClassGeneratorLib
{
    ...
    {
        public JsonClassGenerator(ICodeWriter codeWriter)
        {
        ...
        private readonly Pluralizer _pluralizationService = new Pluralizer();

        private bool _used = false;
        public bool UseNamespaces => Namespace != null;

        public void GenerateClasses(Document document)
        {
            EntityType = document.GetPropertyValue<string>("Bucket");

            string example = JsonConvert.SerializeObject(document);

            if (CodeWriter == null) CodeWriter = new CSharpCodeWriter();

            if (MainClass == null) MainClass = EntityType;

            // if (TargetFolder == null) TargetFolder = "/Code/classes";
            ...
            {
                WriteClassesToFile(StringBuilder, Types);
            }
        ...
        private void WriteClassesToFile(string path, IEnumerable<JsonType> types)
        {
            ...
            //{
                WriteClassesToFile(new StringBuilder(), types);
            //}
        }
        ...
        public void GenerateClass(JObject[] examples, JsonType type)
        {
